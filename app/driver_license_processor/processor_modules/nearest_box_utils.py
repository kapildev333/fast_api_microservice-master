...
class NearestBox:
    ...
    def searchNearestBoundingBoxes(self, box_coordinates, box_indexes, img):
        ...
        for i, left_box_centers in enumerate(left_centers_box_full):
            right_centers_distance1[i] = np.linalg.norm(
                right_centers[0] - left_box_centers)  # box1 right center - other boxes left center
        ...
        for i, right_box_centers in enumerate(right_centers_box_full):
            left_centers_distance1[i] = np.linalg.norm(
                left_centers[0] - right_box_centers)  # box1 left center - other boexes right center
        ...
        box1_r_neighbours = np.where(
            np.all(right_centers_distance1 > 0, axis=1) & np.all(right_centers_distance1 < [self.DISTANCE_THRESH],
                                                                 axis=1))
        box2_r_neighbours = np.where(
            np.all(right_centers_distance2 > 0, axis=1) & np.all(right_centers_distance2 < [self.DISTANCE_THRESH],
                                                                 axis=1))
        box3_r_neighbours = np.where(
            np.all(right_centers_distance3 > 0, axis=1) & np.all(right_centers_distance3 < [self.DISTANCE_THRESH],
                                                                 axis=1))
        box4_r_neighbours = np.where(
            np.all(right_centers_distance4 > 0, axis=1) & np.all(right_centers_distance4 < [self.DISTANCE_THRESH],
                                                                 axis=1))

        box1_l_neighbours = np.where(
            np.all(left_centers_distance1 > 0, axis=1) & np.all(left_centers_distance1 < [self.DISTANCE_THRESH],
                                                                axis=1))
        box2_l_neighbours = np.where(
            np.all(left_centers_distance2 > 0, axis=1) & np.all(left_centers_distance2 < [self.DISTANCE_THRESH],
                                                                axis=1))
        box3_l_neighbours = np.where(
            np.all(left_centers_distance3 > 0, axis=1) & np.all(left_centers_distance3 < [self.DISTANCE_THRESH],
                                                                axis=1))
        box4_l_neighbours = np.where(
            np.all(left_centers_distance4 > 0, axis=1) & np.all(left_centers_distance4 < [self.DISTANCE_THRESH],
                                                                axis=1))
        ...
        return np.asarray([target_box_cordinates["box1"], target_box_cordinates["box2"], target_box_cordinates["box3"],
                           target_box_cordinates["box4"]])
    ...
    def drawlineBetweenBox(self, BoxNum, right_centers, left_centers_box_full, box2_r_neighbours, img):
        ...
        end_point = int(left_centers_box_full[box2_neighbour_indexes][0]), int(
            left_centers_box_full[box2_neighbour_indexes][1])
